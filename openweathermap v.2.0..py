...
def show_weather():
    ...
    if not cityname:
        ...
        if response.status_code != 404:
            ...
            picture.image = img
            weather.config(text=f"{(response["weather"][0]["description"]).capitalize()}\n"
                            ...
                            f"Закат солнца: {datetime.fromtimestamp(response["sys"]["sunset"]).strftime("%H:%M")}")
        else:
            weather.config(text="")
            picture.config(image="")
...
prompting_message = Label(canvas, text="Укажите город", font=("Times New Roman", 18))
canvas.create_window(225, 35, window=prompting_message, anchor="center")
...
picture = Label(canvas)
canvas.create_window(225, 195, window=picture, anchor="center")

weather = Label(canvas, font=("Times New Roman", 12))
canvas.create_window(225, 350, window=weather, anchor="center")
